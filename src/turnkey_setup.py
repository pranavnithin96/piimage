...
class PowerMonitorSetup:
    ...
    def get_hardware_id(self):
        """Get hardware-based ID for fallback"""
        try:
            ...
            with open('/proc/cpuinfo', 'r') as f:
                for line in f:
                    if line.startswith('Serial'):
                        ...
                        return serial[-8:]  # Last 8 characters
        except:
            pass
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return mac[-8:]  # Last 8 characters
        except:
            pass
            ...
            for service in services:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Extract timezone from different services
                        if 'timezone' in data:
                            return data['timezone']  # worldtimeapi
                        elif 'timezone' in data:
                            return data['timezone']  # ipapi.co
                        elif 'timezone' in data:
                            return data['timezone']  # ip-api.com
...
DETECTED_TIMEZONE = "{config['TIMEZONE']}"
SEND_INTERVAL = 10
...
def device_configuration_screen(stdscr, setup):
    """Enhanced device configuration screen with custom device name"""
    config = {
        'DEVICE_ID': '',  # User will enter this
        'LOCATION_NAME': '',
        'CT_RATING': 100,
        'VOLTAGE': 120.0,
        'SERVER_URL': 'https://linesights.com/api/data',
    ...
    stdscr.clear()
    draw_header(stdscr, "🔌 Detecting Location & Timezone...")
    stdscr.addstr(4, 2, "Detecting your location and timezone...")
    stdscr.addstr(5, 2, "This may take a few seconds...")
    stdscr.refresh()
    
    location, detected_timezone = setup.get_location_info()
    ...
    # Suggest device ID based on location
    suggested_device_id = f"powermon_{location.replace(' ', '_').replace(',', '').lower()}"
    if len(suggested_device_id) > 30:
        suggested_device_id = suggested_device_id[:30]
    ...
    while True:
        ...
        stdscr.addstr(4, 2, f"📍 Detected Location: {location}", curses.A_DIM)
        stdscr.addstr(5, 2, f"🕐 Detected Timezone: {detected_timezone}", curses.A_DIM)
        
        # Device ID
        attr = curses.A_REVERSE if current_field == 0 else curses.A_NORMAL
        stdscr.addstr(7, 2, "Device Name: ", attr)
        device_display = config['DEVICE_ID'] or f"[Enter device name, e.g. {suggested_device_id[:25]}...]"
        stdscr.addstr(7, 15, device_display, attr)
        
        # Location Name
        attr = curses.A_REVERSE if current_field == 1 else curses.A_NORMAL
        stdscr.addstr(9, 2, "Location Name: ", attr)
        stdscr.addstr(9, 17, config['LOCATION_NAME'] or "[Enter location name]", attr)
        
        # CT Rating
        attr = curses.A_REVERSE if current_field == 2 else curses.A_NORMAL
        stdscr.addstr(11, 2, "CT Sensor Rating: ", attr)
        ct_display = f"{config['CT_RATING']}A (Use ←→ to change)"
        stdscr.addstr(11, 20, ct_display, attr)
        
        # Voltage
        attr = curses.A_REVERSE if current_field == 3 else curses.A_NORMAL
        stdscr.addstr(13, 2, "Grid Voltage: ", attr)
        voltage_display = f"{config['VOLTAGE']:.0f}V (Use ←→ to change)"
        stdscr.addstr(13, 16, voltage_display, attr)
        
        # Server URL
        attr = curses.A_REVERSE if current_field == 4 else curses.A_NORMAL
        stdscr.addstr(15, 2, "Server URL: ", attr)
        stdscr.addstr(15, 14, config['SERVER_URL'], attr)
        
        # Instructions
        stdscr.addstr(17, 2, "Navigation:", curses.A_BOLD)
        stdscr.addstr(18, 2, "↑↓ - Select field  |  ENTER - Edit  |  ←→ - Change options  |  F1 - Finish Setup")
        
        # Device naming guide
        stdscr.addstr(20, 2, "Device Naming Guide:", curses.A_BOLD)
        stdscr.addstr(21, 2, "Use descriptive names like: kitchen_main, garage_panel, shop_meter")
        stdscr.addstr(22, 2, "Avoid spaces and special characters. Use underscores for readability.")
        ...
        if key == curses.KEY_UP and current_field > 0:
            ...
            if current_field == 0:  # Device ID
                stdscr.addstr(24, 2, f"Enter device name (suggestion: {suggested_device_id}): ")
                stdscr.refresh()
                curses.echo()
                device_id = stdscr.getstr().decode('utf-8').strip()
                curses.noecho()
                if device_id:
                    # Clean device ID - remove spaces and special chars
                    device_id = ''.join(c for c in device_id if c.isalnum() or c in '_-').lower()
                    config['DEVICE_ID'] = device_id
            elif current_field == 1:  # Location Name
                stdscr.addstr(24, 2, "Enter location name: ")
                stdscr.refresh()
                curses.echo()
                location_name = stdscr.getstr().decode('utf-8').strip()
                curses.noecho()
                if location_name:
                    config['LOCATION_NAME'] = location_name
            elif current_field == 4:  # Server URL
                stdscr.addstr(24, 2, "Enter server URL: ")
                stdscr.refresh()
                curses.echo()
            ...
            if config['DEVICE_ID'] and config['LOCATION_NAME']:
                return config
            else:
                show_message(stdscr, "❌ Please enter both device name and location before finishing!", 2)

# [Include all the other functions from the previous version: wifi_setup_screen, scan_and_connect_wifi, etc.]
# [The main_setup_flow and main functions remain the same]
...
def final_setup_screen(stdscr, setup, config, action):
    """Final setup and installation screen with different actions"""
    stdscr.clear()
    
    if action == "apply":
        draw_header(stdscr, "🔌 Power Monitor Setup - Apply Configuration")
        action_text = "Apply changes and restart service"
    else:  # action == "save"
        draw_header(stdscr, "🔌 Power Monitor Setup - Save Configuration") 
        action_text = "Save configuration (service will use on next restart)"
    
    stdscr.addstr(4, 2, "Configuration Summary:", curses.A_BOLD)
    stdscr.addstr(6, 4, f"Device Name: {config['DEVICE_ID']}")
    stdscr.addstr(7, 4, f"Location: {config['LOCATION_NAME']}")
    stdscr.addstr(8, 4, f"Timezone: {config['TIMEZONE']}")
    stdscr.addstr(9, 4, f"CT Rating: {config['CT_RATING']}A (all 6 sensors)")
    stdscr.addstr(10, 4, f"Grid Voltage: {config['VOLTAGE']:.0f}V")
    stdscr.addstr(11, 4, f"Server URL: {config['SERVER_URL']}")
    
    stdscr.addstr(13, 2, f"Action: {action_text}", curses.A_BOLD)
    stdscr.addstr(15, 2, "Press ENTER to proceed, ESC to go back...")
    stdscr.refresh()
    ...
    if key == 27:  # ESC
        ...
        if action == "apply":
            stdscr.addstr(17, 2, "Applying configuration changes...")
        else:
            stdscr.addstr(17, 2, "Saving configuration...")
        stdscr.refresh()
        ...
        stdscr.addstr(18, 2, "✅ Configuration saved")
        stdscr.refresh()
        ...
        stdscr.addstr(19, 2, "✅ Power monitor script updated")
        stdscr.refresh()
        ...
        stdscr.addstr(20, 2, "✅ Service installed and enabled")
        stdscr.refresh()
        
        # Mark setup complete
        setup.mark_setup_complete()
        stdscr.addstr(21, 2, "✅ Setup marked as complete")
        stdscr.refresh()
        ...
        if action == "apply":
            ...
            try:
                ...
                stdscr.addstr(22, 2, "✅ Service restarted with new configuration!")
            except:
                stdscr.addstr(22, 2, "⚠️  Configuration saved but service restart failed")
        else:
            stdscr.addstr(22, 2, "✅ Configuration saved! Restart service to apply changes.")
        
        stdscr.refresh()
        
        stdscr.addstr(24, 2, "🎉 Configuration Complete!", curses.A_BOLD)
        stdscr.addstr(25, 2, f"Device '{config['DEVICE_ID']}' configured for: {config['LOCATION_NAME']}")
        if action == "apply":
            stdscr.addstr(26, 2, "Monitor status with: journalctl -u powermonitor -f")
        else:
            stdscr.addstr(26, 2, "Restart service with: sudo systemctl restart powermonitor")
        stdscr.addstr(27, 2, "Press any key to exit...")
        stdscr.refresh()
        stdscr.getch()
...
def main_setup_flow(stdscr):
    """Main setup flow - always accessible for configuration management"""
    ...
    stdscr.clear()
    if config_exists:
        draw_header(stdscr, "🔌 Power Monitor Configuration Manager")
        stdscr.addstr(4, 2, "Welcome back! You can view and modify your current configuration.")
        stdscr.addstr(6, 2, "Current Configuration:")
        stdscr.addstr(7, 4, f"📱 Device: {config.get('DEVICE_ID', 'Not set')}")
        stdscr.addstr(8, 4, f"📍 Location: {config.get('LOCATION_NAME', 'Not set')}")
        stdscr.addstr(9, 4, f"🕐 Timezone: {config.get('TIMEZONE', 'Not set')}")
        stdscr.addstr(10, 4, f"⚡ Voltage: {config.get('VOLTAGE', 'Not set')}V")
        stdscr.addstr(11, 4, f"🔌 CT Rating: {config.get('CT_RATING', 'Not set')}A")
        
        stdscr.addstr(13, 2, "Options:")
        stdscr.addstr(14, 4, "• View and modify any setting")
        stdscr.addstr(15, 4, "• Save changes and restart service immediately")
        stdscr.addstr(16, 4, "• Save changes for next service restart")
    else:
        ...
        stdscr.addstr(6, 2, "Features:")
        stdscr.addstr(7, 4, "• Custom device naming")
        stdscr.addstr(8, 4, "• Automatic timezone detection")
        stdscr.addstr(9, 4, "• WiFi configuration")
        stdscr.addstr(10, 4, "• 6-CT sensor monitoring")
    
    stdscr.addstr(18, 2, "Press ENTER to continue, ESC to exit...")
    stdscr.refresh()
    ...
    # WiFi Setup (only for new configurations)
    if not config_exists:
        if not wifi_setup_screen(stdscr, setup):
            return
    
    # Device Configuration (always shown)
    config_result = device_configuration_screen(stdscr, setup)
    if config_result is None:
        return
    
    config, action = config_result
    if not config:
        return
    ...
    # Final Setup
    final_setup_screen(stdscr, setup, config, action)
